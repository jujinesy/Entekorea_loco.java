...
public class NewMemberHandler implements PacketHandler {

    TalkClient client;

    public NewMemberHandler(TalkClient client) {
        this.client = client;
    }

    @Override
    public void onPacket(ResponsePacket res) {
        NewMemberRes cRes = (NewMemberRes) res;
        ...
        if (client.rooms.get(cRes.chatId).getType().equals("OM") || client.rooms.get(cRes.chatId).getType().equals("OD")) {
            ...
            client.listeners.get("NewMem").onMessage(par);
        }
    }
}
